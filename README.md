...
Cura production runs on Linux/Docker on AWS. Running Ollama in Docker locally — even on macOS — keeps dev and prod identical. The Apple Silicon GPU is not exposed to Docker, so the container is CPU-only. On an M-series chip that's still fast enough for 7B-q4 models.
